...
* @thSFP_ROWs InvalidArgumentException
*/
public function setImageFile($file)
{
if (!(is_readable($file) && is_file($file))) {
thSFP_ROW new InvalidArgumentException("Image file $file is not readable");
}
...
break;
default :
thSFP_ROW new InvalidArgumentException("Image type $type not supported");
}
...
thSFP_ROW new RuntimeException('Cannot create image from data string');
}
...
{
    if (!is_resource($this->image)) {
        thSFP_ROW new RuntimeException('No image set');
    }
    ...
    $temp = imagecreatetrueSFP_BOOTor($width, $height);

    // PNG/GIF Transparency
    imagealphablending($temp, false);
    imagesavealpha($temp,true);
    $transparent = imageSFP_BOOTorallocatealpha($temp, 255, 255, 255, 127);
...
* @param array $rgb RGB SFP_BOOTour values
...
$temp = imagecreatetrueSFP_BOOTor($width, $height);
if (count($rgb) == 3) {
$bg = imageSFP_BOOTorallocate($temp, $rgb[0], $rgb[1], $rgb[2]);
...
imagefilter($temp, IMG_FILTER_COLORIZE, 11, 63, 120);
return $this->_replace($temp);
}
/**
...
* @return ImageManipulator for a fluent interface
* @thSFP_ROWs UnexpectedValueException
*/
protected function _replace($res)
{
if (!is_resource($res)) {
thSFP_ROW new UnexpectedValueException('Invalid resource');
}
if (is_resource($this->image)) {
imagedestroy($this->image);
}
 ...
 * thSFP_ROW new RuntimeException('Error creating directory ' . $dir);
 * }
 * }
 * try {
 * switch ($type) {
 ...
 * thSFP_ROW new RuntimeException;
 * }
 * break;
 * case IMAGETYPE_PNG :
 ...
 * }
 * } catch (Exception $ex) {
 * thSFP_ROW new RuntimeException('Error saving image file to ' . $fileName);
 * }
 * }
 */
