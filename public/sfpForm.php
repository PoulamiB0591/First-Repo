...
$sql1 = mysql_fetch_assoc(mysql_query("select * from  `userTemplate` where `campaign_number`='".$camNo."' and `new_template_name` = '".$camFolder."'")) ;
...
{
    $sqlCount = mysql_fetch_assoc(mysql_query("SELECT MAX(count) AS HighestCount FROM `formreport` WHERE `form_uniqid`='".$formId."'"));
    $sqlForm = mysql_query("SELECT * FROM `formreport` WHERE `form_uniqid`='".$formId."' AND `count`='".$sqlCount['HighestCount']."'");
