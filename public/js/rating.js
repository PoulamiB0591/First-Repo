...
$(document).ready(function() {
  ...
  $('#example-2').ratings(5).bind('ratingchanged', function(event, data) {
    ...
    var hidtemp = $("#hiddentemp").val();
         $.ajax({
            type:'POST',
            data:{tempid:hidtemp,rate:res},
            url:base_url+"/index/rating",
            success:function(data){
           
                if(data == "0")
                {
                    alert("you cannot rate it anymore");
                    return false;
                }
                else if(data == "1")
                {
                    alert("you have to register to rate this template");
                    window.location.href = base_url+"/index/accountsignup";
                    return false;
                }
                else
                {
               var data1 = data.split("|-|");
              $('#example-rating-2').text(data1[0]+" average based on "+data1[1]+" ratings");   
                }
    ...
    var base_url = window.location.origin;
         ...
         $.ajax({
            ...
            success:function(data){
              ...
              var data1 = data.split("|-|");
              $('#example-rating-3').text(data1[0]+" average based on "+data1[1]+" ratings");
            }
        });
    clk++;
  });    
