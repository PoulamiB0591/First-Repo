...
chmod($userfolder."/upload", 0777);
  ...
  foreach (glob($userfolder."/*.php") as $search) {
    $contents = file_get_contents($search);
    if($search != $userfolder."index.php"){
    if (!strpos($contents, "/Apps/effdfsf/151/index.php")) continue;
   
    str_replace("https://test.scampaigns.com/","https://".$_SERVER['SERVER_NAME']."/",$contents);
    str_replace("/Apps/effdfsf/151/","/Apps/".$values2."/".$values."/",$contents);
   $file_contents = str_replace("/Apps/effdfsf/151/index.php",$getindex,$contents);
    file_put_contents($search,$file_contents); 
...
foreach (glob($userfolder."/*.php") as $search) {
   ...
   $contents = str_replace("/var/www/staging/public/Apps/effdfsf/151",$_SERVER['DOCUMENT_ROOT']."/Apps/".$values2."/".$values,$contents);
   $contents =  str_replace("https://test.scampaigns.com/","https://".$_SERVER['SERVER_NAME']."/",$contents); 
   $contents = str_replace("/Apps/effdfsf/151/","/Apps/".$values2."/".$values."/",$contents);
   $contents = str_replace("script","script",$contents);
    $contents=str_replace(" "," ",$contents);
  $contents=str_replace(""," ",$contents);
 ...
 $str = str_replace("/Apps/effdfsf/151/","/Apps/".$values2."/".$values."/",$str);
  $str=str_replace("/Apps/effdfsf/151/index.php",$getindex,$str);
 $str=str_replace("/var/www/staging/public/Apps/effdfsf/151",$_SERVER['DOCUMENT_ROOT']."/Apps/".$values2."/".$values,$str);
 $str = str_replace(" "," ",$str);
...
chmod($pathfolder1, 0777);
   ...
   $file_contents = str_replace("/Apps/effdfsf/151/","/Apps/".$values2."/".$values."/",$contents); 
