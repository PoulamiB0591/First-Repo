...
chmod($userfolder."/upload", 0777);
  ...
  foreach (glob($userfolder."/*.php") as $search) {
    $contents = file_get_contents($search);
    if($search != $userfolder."index.php"){
    if (!strpos($contents, "/Apps/dsdsg_upload/132/index.php")) continue;
   
    //str_replace("SFP_Redirect/","mobileview/",$contents);
   $file_contents = str_replace("/Apps/dsdsg_upload/132/index.php",$getindex,$contents);
    file_put_contents($search,$file_contents); 
...
foreach (glob($userfolder."/*.php") as $search) {
   ...
   $contents = str_replace("/var/www/staging/public/Apps/dsdsg_upload/132/",$_SERVER['DOCUMENT_ROOT']."/Apps/".$values2."/".$values."/",$contents);
    //str_replace("SFP_Redirect/","mobileview/",$contents); 
   $contents = str_replace("script","script",$contents);
    $contents=str_replace(" "," ",$contents);
  $contents=str_replace(""," ",$contents);
 ...
 //$str=str_replace("SFP_Redirect/",$userfilename,$str);
  $str=str_replace("/Apps/dsdsg_upload/132/index.php",$getindex,$str);
 $str=str_replace("/var/www/staging/public/Apps/dsdsg_upload/132/",$_SERVER['DOCUMENT_ROOT']."/Apps/".$values2."/".$values."/",$str);
 $str = str_replace(" "," ",$str);
