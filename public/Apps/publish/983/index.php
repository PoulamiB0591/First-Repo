...
//echo "select * from  `userTemplate` where `campaign_number`='".$values."' and `new_template_name` = '".$values2."'";exit;
$query = mysql_query("select * from  `userTemplate` where `campaign_number`='".$values."' and `new_template_name` = '".$values2."'");

$sql1 = mysql_fetch_assoc($query) ;
...
if($sql['published'] == 0)
{
    echo "SORRY YOUR TEMPLATE IS NOT YET PUBLISHED !!!!";
    exit;
}
?>
<title><?php
if (isset($pageTitle)) {
echo str_replace("_"," ",$pageTitle);
} else {
