...
chmod($userfolder."/upload", 0777);
  ...
  foreach (glob($userfolder."/*.php") as $search) {
    $contents = file_get_contents($search);
    if($search != $userfolder."index.php"){
    if (!strpos($contents, "/Apps/Linkgenerator/670/index.php")) continue;
   
    //str_replace("SFP_Redirect/","mobileview/",$contents);
   $file_contents = str_replace("/Apps/Linkgenerator/670/index.php",$getindex,$contents);
    file_put_contents($search,$file_contents); 
...
foreach (glob($userfolder."/*.php") as $search) {
  ...
  $contents=str_replace("",$sql['appid'],$contents);
  $contents=str_replace("",$sql['appsecret'],$contents);
  $contents = str_replace("","",$contents);
  $contents = str_replace("","",$contents);
 ...
 //$str=str_replace("SFP_Redirect/",$userfilename,$str);
  $str=str_replace("/Apps/Linkgenerator/670/index.php",$getindex,$str);
 $str=str_replace("mobileview/","mobileview/".$userfilename,$str);
 $str = str_replace(" "," ",$str);
 $str=str_replace('email,publish_actions,public_profile,',$var,$str);
 $str=str_replace("",$sql['appid'],$str);
 $str=str_replace("",$sql['appsecret'],$str);
 $str = str_replace("","",$str);
 $str = str_replace("","",$str);
...
chmod($pathfolder, 0777);
   ...
   $file_contents = str_replace('',$sql['appid'],$contents);
   $file_contents = str_replace('',$sql['appsecret'],$contents);
