...
chmod($userfolder."/upload", 0777);
  ...
  foreach (glob($userfolder."/*.php") as $search) {
    $contents = file_get_contents($search);
    if($search != $userfolder."index.php"){
    if (!strpos($contents, "/Apps/dgdg/343/index.php")) continue;
   
    str_replace("https://test.scampaigns.com/","https://".$_SERVER['SERVER_NAME']."/",$contents);
   $file_contents = str_replace("/Apps/dgdg/343/index.php",$getindex,$contents);
    file_put_contents($search,$file_contents); 
...
foreach (glob($userfolder."/*.php") as $search) {
   ...
   $contents = str_replace("/var/www/staging/public/Apps/dgdg/343",$_SERVER['DOCUMENT_ROOT']."/Apps/".$values2."/".$values,$contents);
 ...
 $str=str_replace("https://test.scampaigns.com/","https://".$_SERVER['SERVER_NAME']."/",$str);
  $str=str_replace("/Apps/dgdg/343/index.php",$getindex,$str);
 $str=str_replace("/var/www/staging/public/Apps/dgdg/343",$_SERVER['DOCUMENT_ROOT']."/Apps/".$values2."/".$values,$str);
 $str = str_replace(" "," ",$str);
