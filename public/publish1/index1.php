...
$sql = mysql_fetch_assoc(mysql_query("select * from  `userTemplate` where `template_link`='830-epic'")) ;

$sessionid = session_id();


mkdir( $_SERVER['DOCUMENT_ROOT']."/publish1/uid-".$sessionid,0777,true);
chmod($_SERVER['DOCUMENT_ROOT']."/publish1/uid-".$sessionid, 0777);
fopen($_SERVER['DOCUMENT_ROOT']."/temporary/temp-".$sessionid.".php", "w");
$userfilename ="uid-".$sessionid."/";
$userfolder = $_SERVER['DOCUMENT_ROOT']."/publish1/uid-".$sessionid;
$tempfile = $_SERVER['DOCUMENT_ROOT']."/temporary/temp-".$sessionid.".php";
$file_handle = fopen($_SERVER['DOCUMENT_ROOT']."/user_template/830-epic/index.php", "r");
$file_handle1 = fopen($tempfile, "rw+");
$src = $_SERVER['DOCUMENT_ROOT']."/user_template/830-epic";
$varq = $_SERVER['DOCUMENT_ROOT']."/user_template/830-epic/index.php";
$dst = $_SERVER['DOCUMENT_ROOT']."/publish1/uid-".$sessionid;

$files = glob($_SERVER['DOCUMENT_ROOT']."/user_template/830-epic/*.*");
      foreach($files as $file){
        //print_r($files);
       if($file != $_SERVER['DOCUMENT_ROOT'].'/user_template/830-epic/index.php'){
...
foreach (glob($userfolder."/*.php") as $search) {
   ...
   $contents = str_replace("SFPUPLOAD","publish1/".$userfilename,$contents);
    //str_replace("SFP_Redirect/","publish/",$contents);
   $contents = str_replace("!--SFPScript","script",$contents);
...
include $tempfile;
 $pathfolder = $_SERVER['DOCUMENT_ROOT'].'/user_template/830-epic';
