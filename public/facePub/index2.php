...
@require_once ($_SERVER['DOCUMENT_ROOT']."/facePub/facebook/facebook.php");  
   $facebook = new Facebook(array(
			'appId' => '1512765695673590',
			'secret' => 'b47a5f0ad974ca57580a6051f4f5fceb',
			'cookie' => true)
	     );
...
if(is_array( $str )){
       ...
       $sql = mysql_query("select * from  `userTemplate` where `pageId`='".$page_id."' and `appId` = '1512765695673590'");
       
       $arrayData = mysql_fetch_assoc($sql);
      
       $protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https://' : 'http://';
       
       // $protocol.$_SERVER['SERVER_NAME'].'/facebook/pageTab/'.$arrayData['facebook_number'].'/index.php';exit;
