...
//		});
            ...
            $(".wrapper .SFPTWO_EDITFORM ul").find("li").append('<div class="setting-2 prevnext"><a title="delete" class="delete" href="javascript:void(0)"></a><input type="hidden" id="hid" /><input type="hidden" value="0" id="imgCount" /><a class="prev" href="#">Previous</a>&nbsp;<a class="next" href="#">After</a></div>');
...
//		});
      ...
      $("#saveCam").click(function (){
       ...
       // alert(camName);
        $.ajax({
           type : "POST",
           url : "http://test.scampaigns.com/frontend/savename",
           ...
           success : function (data)
           {
               ...
               {
                    $("#url").show();
                    $("#fieldLabel").html('<label id="webUrl"> https://test.scampaigns.com/Apps/'+camName+'/'+camNo+'</label>');
               }
           }
        });
      });	
                ...
                for(var j =1; j<=total; j++)
                {
                   ...
                   // alert(cls);
                    ...
                    if(htm1 == "")
                    {
                        ...
                        var totalchk = $("#totalchk").val();
                        
                       $('.wrapper').find('form > ul > li.'+cls).find('input[type=checkbox]').each(function(){
                        var totchk = "check_"+totalchk;
                        $(this).attr('name',totchk);
                        totalchk++;
                       });
                      $("#totalchk").val(totalchk);      
                     ...
                     if(htm3 == "")
                    {
                        ...
                        var totalradio = $("#totalradio").val();
                        
                       $('.wrapper').find('form > ul > li.'+cls).find('input[type=radio]').each(function(){
                        var totradio = "radio_"+totalradio;
                        $(this).attr('name',totradio);
                        totalradio++;
                       });
                      $("#totalradio").val(totalchk);      
...
function addPages(pagename) {
     ...
     //alert("work now!!!!");
      ...
      $.ajax({
            ...
            url: 'http://test.scampaigns.com/app/pageadded',
            success: function (data) {
                alert(data);
            }
            });
